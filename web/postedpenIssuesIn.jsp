...
String sql = "SELECT * FROM issue WHERE  (issue_status<>'Completed' OR status_byempolyee<>'Completed');";
...
<% session.setAttribute("employeePostedId", rs.getInt("employee_id")); %>
<td><a href="empPostedDetails.jsp">Employee Details</a></td>
            <td><%= rs.getString("issue_status") %></td>
            <td><form name="remarks" method="get" action="remarks.jsp" onsubmit="return verify()">
                <%= rs.getString("remarksByEmp") %></td>
            <td><%= rs.getString("feedbackByEmp") %></td>
            <td><%= rs.getString("status_byempolyee") %></td>
            <td><input type="text" name="remarksByIn" value="<%= rs.getString("remarksByIn") %>"/></td>
